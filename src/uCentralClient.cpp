...
void uCentralClient::Connect1() {
    ...
    try {
        ...
        Connected_ = true ;
        NextConnect_ = 0 ;
        NextState_ = time(nullptr) + Service()->GetStateInterval();
        NextCheck_ = time(nullptr) + Service()->GetHealthCheckInterval();
    }
    catch ( const Poco::Exception & E )
    {
        std::cout << "Could not connect, reconnecting in 10 seconds." << std::endl;
        State_ = initialized;
        NextConnect_ = time(nullptr) + Service()->GetReconnectInterval();
    }
}
...
void uCentralClient::Connect() {
    ...
    Logger_.information( Poco::format("%s: Connecting.", SerialNumber_));
    std::this_thread::sleep_for(1500ms);
    State_ = connected;
}

void uCentralClient::SendConnection() {
    {
        std::lock_guard<std::mutex> guard(mutex_);
        State_ = sending_hello;
    }
    Logger_.information( Poco::format("%s: Sending connection.",SerialNumber_));
...
void uCentralClient::SendState() {
    ...
    Logger_.information( Poco::format("%s: Sending state.", SerialNumber_));
}

void uCentralClient::SendHealthCheck() {
    {
        std::lock_guard<std::mutex> guard(mutex_);
        NextCheck_ = time(nullptr) + Service()->GetHealthCheckInterval();
    }
    Logger_.information( Poco::format("%s: Sending health check.", SerialNumber_));
}

void uCentralClient::SendClosing() {

    Logger_.information( Poco::format("%s: Closing.", SerialNumber_));
}
